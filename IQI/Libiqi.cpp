...
void mad(C_Matrix_Container* input,C_Matrix_Container* output)
{
	// liczy mad dla dla wszystki kolumn. Jeœli tylko jeden rz¹d to liczy dla tego rzêdu
	double med;
	unsigned long a,r,c;
	double* ttmp;
...
void findonevalue(C_Matrix_Container* input,C_Matrix_Container* output,double wartosc)
{
	// znajduje jedn¹ wartoœæ w macierzy. Zwraca macierz 2xn z indeksami znalezionej wartoœci 0xn rzêdy
...
void CutMatrixCol(C_Matrix_Container* input, unsigned int start, unsigned int stop)
{
// wycina po kolumnach od start do stop. Modyfikuje strukture weœciow¹. Zostaj¹ te wyciête np (1:4)
	unsigned int c,r;
	double pixel;
