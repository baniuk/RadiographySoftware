#include "StdAfx.h"
#include "C_Command_Berode.h"

C_Command_Berode::C_Command_Berode(void)
{
	UINT parint[3] = {IMAGE,NUMBER,NUMBER};
	UINT parout[1] = {IMAGE};
	InitCommand("berode",3,1,parint,parout);
	intab = new C_Variables*[numofinparam];
	outtab = new C_Variables*[numofoutparam];
	syntax = "IMAGE = berode(IMAGE,NUMBER,NUMBER)";
	fun_ver = "1.1";
}
...
void C_Command_Berode::Do(C_Error& err)
{
	...
	size_x = (unsigned int)p_sizex->data[0];
	size_y = (unsigned int)p_sizey->data[0];
