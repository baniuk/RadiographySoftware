...
{
	// jeœli st==0 to znaczy ze obiekt tworzony na chwile, jeœli 1 to finalne tworzenie
	char* DLLname;
...
/* Tutaj mamy:	LHP - nazwa parametru lewego
				...
				czysaparametryfunkcji - true jeœli s¹ jakieœ parametry
...
{
/* porównuje parametry wejœciowe przekazane w wywo³aniu funkcji do tych które s¹ zdefiniowane w workspace */	
		...
		if(line->RHPFTYPE[a]&& !line->STRTYPE[a])			// tylko gdy parametr nie jest liczb¹ i ne jest ci¹giem tekstowym
			if(lastvar>0)	{
				for(b=0;b<lastvar;b++)		{					// po tym co w workspace
					znacznik = 0;
					if(strcmp(var[b]->name,line->RHPF[a])==0)	{	// jeœli jest
						znacznik = 1;
						break;
					}
				}
	...
	if(line[num]->FNTYPE)	{	//gdy fn nie jest nazwa funkcji
		...
		if(line[num]->STRTYPE[0])	{	// gdy jest stringiem
			if(index<0)	{	// zmienna nie istnieje - tworzymy
				CreateVariable(line[num],STRING,lastvar,var);	// adres stworzonej zmiennej jest zapamiêtywany w tablicy intab w obiekcie funkcji
				var[lastvar]->AssignName(line[num]->LHPF[0]);
				var[lastvar]->AssignValue((void*)line[num]->FN,err);
				lastvar++;
			}
			else	{
				CreateVariable(line[num],STRING,index,var);	// adres stworzonej zmiennej jest zapamiêtywany w tablicy intab w obiekcie funkcji
			...
			if(index<0)	{	// zmienna nie istnieje - tworzymy
				CreateVariable(line[num],NUMBER,lastvar,var);	// adres stworzonej zmiennej jest zapamiêtywany w tablicy intab w obiekcie funkcji
				var[lastvar]->AssignName(line[num]->LHPF[0]);
				...
				CreateVariable(line[num],NUMBER,index,var);	// adres stworzonej zmiennej jest zapamiêtywany w tablicy intab w obiekcie funkcji
				var[index]->AssignName(line[num]->LHPF[0]);
		...
		if(err.status) return;
		if(line[num]->numofleftparams>0)	{			// jeœli s¹ jakieœ parametry wyjœciowe
			type = tab[lasttab]->ReturnDTout();	// to pobieramy jakie s¹ ich typy
			for(a=0;a<line[num]->numofleftparams;a++)	{	// tworzymy potrzebne zmienne wyjœciowe
				index = GetIndexofVariable(line[num]->LHPF[a]);
				if(index<0)	{	// zmienna nie istnieje - tworzymy
					tab[lasttab]->outtab[a] = CreateVariable(line[num],type[a],lastvar,var);	// adres stworzonej zmiennej jest zapamiêtywany w tablicy intab w obiekcie funkcji
...
int C_InterpBase::DecodeType(UINT type, UINT* tab)
{
	...
	for(a=0;a<licznik;a++)
		tab[a] = tabtmp[a];	// sprawdziæ jak s¹ tablice bez rozmiaru
	return licznik-1;
