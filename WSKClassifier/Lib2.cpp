...
#define NO_VERBOSE
#define MAX_FLAW_AREA 0.8
#define MIN_FLAW_AREA 0.005
...
BOOL isdeleted(unsigned int ind,C_Matrix_Container* do_kasacji);

void Introduce(FUNC_DEF* definition)
{
	strcpy_s(definition->name,MAX_NAME,"wskpreclassifier");	// nazwa w ADDIP
	...
	strcpy_s(definition->syntax,MAX_SYNT,"IMAGE = wskpreclassifier(IMAGE,IMAGE,IMAGE,MATRIX,MATRIX,STRING)"); // SYNTAX (nie ma wiêkszego znaczenia)
...
void Do(C_Error& err, C_Variables** intab,int nargin, C_Variables** outtab, int nargout)
{
	...
	C_Matrix_Container do_kasacji;	// indeksy wad do skasowania
	unsigned int licznik_sasiadow=0,licznik_kasacji=0; // ile jest wad sasiadujacych i ile do skasowania
	FILE *stream;
	char* filename;

	unsigned int current_flaw,current_flaw1,c;
	double odl;
	char buffor[10];
	...
	fprintf_s(stream,"SIZE FILTRATION\n");
/* ------------------------------ 1. WYBIERANIE  NIE WAD --------------------------------------------------*/
	do_kasacji.AllocateData(65535,2);	// tu indeksy wad do kasacji [current flaw, index]
	...
	for(current_flaw=0;current_flaw<features->_rows;current_flaw++)	{
		...
		if( (features->GetPixel(current_flaw,10)*100)/imagesize>MAX_FLAW_AREA )	{
			fprintf_s(stream,"Not classified as flaw due to area larger than %f%% of total image\n",MAX_FLAW_AREA);
			do_kasacji.SetPixel(licznik_kasacji,0,current_flaw);
			do_kasacji.SetPixel(licznik_kasacji++,1,features->GetPixel(current_flaw,1));
			continue;		// jesli wada wiêksza niz 1% obrazka to kwalifikujemy jak nie wadê
		}
		if( (features->GetPixel(current_flaw,10)*100)/imagesize<MIN_FLAW_AREA )	{
			fprintf_s(stream,"Not classified as flaw due to area smaller than %f%% of total image\n",MIN_FLAW_AREA);
			do_kasacji.SetPixel(licznik_kasacji,0,current_flaw);
			do_kasacji.SetPixel(licznik_kasacji++,1,features->GetPixel(current_flaw,1));
			continue;		// jesli wada wiêksza niz 1% obrazka to kwalifikujemy jak nie wadê
		}
		fprintf_s(stream,"Preleminary classified as flaw\n");
	}
/* ------------------------------ 2. KASOWANIE  NIE WAD --------------------------------------------------*/
	for(current_flaw=0;current_flaw<licznik_kasacji;current_flaw++)
		for(c=0;c<indexed->GetNumofElements();c++)
			if(indexed->data[c]==do_kasacji.GetPixel(current_flaw,1))
				result.data[c]=0.0;
// jest jakis b³¹d w indeksowaniu i jesli wada przylega do brzegu to zostawiana jest 2 pix obwódka!! (nie jest ona indexowna
	for(c=0;c<indexed->_cols;c++)	{
		result.SetPixel(0,c,0.0);
		result.SetPixel(1,c,0.0);
		result.SetPixel(indexed->_rows-1,c,0.0);
		result.SetPixel(indexed->_rows-2,c,0.0);
	}
	for(c=0;c<indexed->_rows;c++)	{
		result.SetPixel(c,0,0.0);
		result.SetPixel(c,1,0.0);
		result.SetPixel(c,indexed->_cols-1,0.0);
		result.SetPixel(c,indexed->_cols-2,0.0);
	}
#ifdef VERBOSE			
	result.DumpBinary("wsk_result_deleted.out");	
	bw->DumpBinary("wsk_bw.out");
	indexed->DumpBinary("wsk_indexed.out");
#endif
/* ------------------------------ 3. £¥CZENIE WAD --------------------------------------------------*/
	connections.AllocateData(65535,2);	// dla bezpieczenstwa razy 2 gdy kilka wad bedzie sasiadowalo
	connections.Zeros();
	fprintf_s(stream,"\n\n\n###########################################################\n");
	fprintf_s(stream,"CONNECTIONS TABLE\n");
	for(current_flaw=0;current_flaw<features->_rows-1;current_flaw++)	{
		fprintf_s(stream,"-----------------------------------------------------------\n");
		fprintf_s(stream,"Flaw index: %d\n",(unsigned int)features->GetPixel(current_flaw,1));
		if(isdeleted(current_flaw,&do_kasacji))	{	// jesli zaznaczona jako nie wada to nie bierzemy pod uwagê
			fprintf_s(stream,"This flaw is deleted\n");
			continue;
		}
		for(current_flaw1=current_flaw+1;current_flaw1<features->_rows;current_flaw1++)	// kazda z kazd¹
		{
//		current_flaw1=56; //debug
			if(isdeleted(current_flaw1,&do_kasacji))		// jesli zaznaczona jako nie wada to nie bierzemy pod uwagê
				continue;
			odl = dist->GetPixel(current_flaw1,current_flaw);	// odleglosc z funkcji kamila - odleg³oœæ pomiêdzy dwiema wadami 
			if(odl==0.0)
				continue;
/*debug*/	if(kryterium3223(current_flaw,current_flaw1,odl,features))
			{
...
//				if(!wada2.ReadBinary(buffor)) { err.SetError("wskclassifier: File not found"); return;}
				...
				merge(current_flaw,
						   ...
						   &x2,&y2,bw,
						   &result,&result,
						   &out,&outx,&outy);	// out,outc,outy s¹ nieokreslone na skutek bledu
...
BOOL isdeleted(unsigned int ind,C_Matrix_Container* do_kasacji)
{
// sprawdza czy dana wada jest w indeksach skasowanych
	unsigned int a;
	for(a=0;a<do_kasacji->_rows;a++)
		if(ind==do_kasacji->GetPixel(a,0))
			return TRUE;
	return FALSE;
}
...
// nie dzia³a poprawnie - poprawiæ ( zle liczone pole bo z out które jest poste
/*
...
BOOL kryterium3223(unsigned int ind1,unsigned int ind2,double dist,C_Matrix_Container* feat)
{
	double krk,krd,dlk,dld;

	if(feat->GetPixel(ind1,10) > feat->GetPixel(ind2,10))	{
		krk = feat->GetPixel(ind2,30);			// krótszy wymiar mniejszego wskazania
		krd = feat->GetPixel(ind2,29);			// d³ó¿szy wymiar mniejszego wskazania
		dlk = feat->GetPixel(ind1,30);			// krótszy wymiar wiekszego wskazania
		dld = feat->GetPixel(ind1,29);			// d³ó¿szy wymiar wiekszego wskazania
	}
	else	{
		krk = feat->GetPixel(ind1,30);
		krd = feat->GetPixel(ind1,29);
		dlk = feat->GetPixel(ind2,30);			// krótszy wymiar wiekszego wskazania
		dld = feat->GetPixel(ind2,29);			// d³ó¿szy wymiar wiekszego wskazania
	}

	if(dist-(dld/2+krd/2)<krd)		// nie wiadomo co znaczy ta odleglosc kamila?? od brzegu, œrodka czy jak?
		return TRUE;
	else
		return FALSE;
